...
@pytest.mark.parametrize("order_data, should_pass", [
    (OrderCreate(customer_name="John Doe", item="Laptop", quantity=1, total_price=1000.00, status="pending"), True),
    ({"customer_name": 1, "item": 1, "quantity": "one", "total_price": 1000, "status": 1}, False)
])
@patch("app.services.order_service.validate_order_data", return_value=True)
@patch("app.services.order_service.send_to_queue")
def test_create_order(mock_send_to_queue, mock_validate_order_data, mock_db_session, order_data, should_pass):
    if should_pass:
        ...
        order = create_order(mock_db_session, order_data)

        mock_validate_order_data.assert_called_once_with(order_data)
